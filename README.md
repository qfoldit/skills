...
| `skills/uefn-fortnite-world-builder` | UEFN (Unreal Editor for Fortnite) / Fortnite islands | ⚠️ Community only -- Epic has no published UEFN MCP today (roadmap: merges into Unreal Engine 6, targeting late 2027). Primary MCP backends: [KirChuvakov/uefn-mcp-server](https://github.com/KirChuvakov/uefn-mcp-server) (MIT, 28 tools) and [quangdang46/uefn-verse-mcp](https://github.com/quangdang46/uefn-verse-mcp) (AGPL-3.0 + attribution term, 354 actions). <br> <br>  **Additionally, this skill can leverage [undergroundrap/UEFN-TOOLBELT](https://github.com/undergroundrap/UEFN-TOOLBELT)** as a powerful external toolkit (not an MCP server itself, but provides a Python library + CLI + MCP bridge) with 358 tools, autonomous arena/level generation, Verse snippet generation, full world state export, device catalog scanning, and undo‑safe operations. UEFN-TOOLBELT is MIT-licensed and can be used as an alternative backend for complex scene assembly from UAG, especially when the primary MCP servers are insufficient. See the UEFN note below for integration details. |
...
**Note on UEFN/Fortnite:** unlike the three engines above, there is no official-vs-community *pair* here -- only community options exist, since Epic has not published an MCP for UEFN itself (only for standalone Unreal Engine 5.8+, a different product). `claude_desktop_config.json`'s `uefn` entry configures `KirChuvakov/uefn-mcp-server` (MIT) by default, with `quangdang46/uefn-verse-mcp` as an alternative for richer Verse generation. 

**UEFN-TOOLBELT integration:** This skill also supports using `undergroundrap/UEFN-TOOLBELT` (MIT) as a heavy‑duty backend when the MCP servers are insufficient. UEFN-TOOLBELT provides:

- **358 tools** (arena generation, stamp placement, project setup, world state export, device catalog scan, Verse snippet generator, etc.)
- **Autonomous level assembly** from a UAG (via `arena_generate` and `stamp_place`).
- **Full world state export** (500+ actors and properties) for AI analysis.
- **Verse code generation** that compiles without errors.
- **MCP bridge** (`mcp_bridge/`) ready for Claude Desktop.
- **Undo‑safe** operations via `ScopedEditorTransaction` and 163 integration tests.
- **Zero outbound network calls** and no writes outside the project.

To use UEFN-TOOLBELT with this skill, clone the repository alongside the skill folder and set `UEFN_TOOLBELT_PATH` in the skill's environment or config. The skill will fall back to the MCP servers if the toolkit is not found. For full details, see [UEFN-TOOLBELT's README](https://github.com/undergroundrap/UEFN-TOOLBELT).
